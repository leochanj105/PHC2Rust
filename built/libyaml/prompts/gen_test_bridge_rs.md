...
1. **Use the types and module paths that actually exist in `__RUST_DIR__/src/`.**
...
3. **Do NOT modify any file other than the output.** You are not allowed to
   edit `__RUST_DIR__/src/*.rs`. Visibility promotion (making private `fn`
...
4. **Verify the file compiles.** Since you cannot edit `__RUST_DIR__/`, do
   the verification in a scratch copy:
   - `cp -r __RUST_DIR__ /tmp/bridge_verify`
