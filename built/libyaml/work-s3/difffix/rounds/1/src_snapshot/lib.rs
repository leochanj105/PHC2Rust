...
pub unsafe extern "C" fn yaml_emitter_set_encoding(
    ...
    assert!(!emitter.is_null());
    assert!((*emitter).encoding == YAML_ANY_ENCODING);
...
pub unsafe extern "C" fn yaml_document_start_event_initialize(
    ...
    if tag_directives_start != tag_directives_end {
        ...
        while td != tag_directives_end {
            assert!(!(*td).handle.is_null());
            assert!(!(*td).prefix.is_null());
...
pub unsafe extern "C" fn yaml_document_append_sequence_item(
    ...
    assert!((*(*document).nodes.start.add(sequence as usize - 1)).type_ == YAML_SEQUENCE_NODE);
    assert!(item > 0
...
pub unsafe extern "C" fn yaml_document_append_mapping_pair(
    ...
    assert!((*(*document).nodes.start.add(mapping as usize - 1)).type_ == YAML_MAPPING_NODE);
...
pub(crate) unsafe fn yaml_parser_loader_set_composer_error_context(
    parser: *mut yaml_parser_t,
    context: *const c_char,
    context_mark: yaml_mark_t,
    problem: *const c_char,
...
pub(crate) unsafe fn loader_delete_aliases(parser: *mut yaml_parser_t) {
...
unsafe fn yaml_parser_load_sequence_end(
    ...
    assert!((*(*(*parser).document).nodes.start.add(index as usize - 1)).type_ == YAML_SEQUENCE_NODE);
...
unsafe fn yaml_parser_load_mapping_end(
    ...
    assert!((*ctx).top.offset_from((*ctx).start) > 0);
    let index = *((*ctx).top.sub(1));
    assert!((*(*(*parser).document).nodes.start.add(index as usize - 1)).type_ == YAML_MAPPING_NODE);
    (*(*(*parser).document).nodes.start.add(index as usize - 1)).end_mark = (*event).end_mark;
    let _ = stack_pop(&mut (*ctx).top);
    1
}
