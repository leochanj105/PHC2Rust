...
def main(argv=None):
    ...
    if args.single_file:
        ...
        # Per-file mode: emit one bridge_<src>.c per source + a single
        # test_bridge.h declaring every wrapper.
        for abs_src, funcs in groups:
            fname = os.path.basename(abs_src)
            out_path = os.path.join(args.out_dir, f"bridge_{fname}")
            emit_bridge(abs_src, out_path, funcs)

        # Header with all declarations
        all_funcs = [f for _, funcs in groups for f in funcs]
        h_lines = [
            "/* AUTO-GENERATED by gen_bridges.py (per-file mode) — do not edit. */",
            "#ifndef TEST_BRIDGE_H",
            "#define TEST_BRIDGE_H",
            "",
            "#include <yaml.h>",
        ...
        h_lines.append("")
        h_lines.append("#endif /* TEST_BRIDGE_H */")
        with open(os.path.join(args.out_dir, "test_bridge.h"), "w") as f:
            f.write("\n".join(h_lines))

        print()
        print(f"Total bridge wrappers generated: {total}/{total_expected}")
        print(f"Output: {args.out_dir}/bridge_*.c  + {args.out_dir}/test_bridge.h")
