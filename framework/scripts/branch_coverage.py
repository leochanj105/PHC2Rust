...
def load_branches(export_json_path, filter_prefix=None):
    """Load all branch entries from llvm-cov export JSON.

    filter_prefix: a path marker (e.g. 'libm/' or 'src/') used to (a) keep only
    files whose path contains it and (b) normalize the stored file path to
    start at the marker. Defaults to the SOURCE_PATH_MARKER env var; empty
    means keep every file and store the full path verbatim.
    """
    if filter_prefix is None:
        filter_prefix = os.environ.get('SOURCE_PATH_MARKER', '')
    ...
    for finfo in d['data'][0]['files']:
        ...
        if filter_prefix and filter_prefix not in fn:
            continue
        # Normalize filename to relative path from marker (if set)
        if filter_prefix and filter_prefix in fn:
            short = fn[fn.index(filter_prefix):]
        else:
            short = fn
